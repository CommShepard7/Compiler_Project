...
type info =
  ...
  | InfoVar of string * typ * int * string * string * string * info list
...
let rec string_of_info info =
  ...
  | InfoVar (n,t,dep,base,f,infoStruct,ia) -> "Variable "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"^" "^f^" struct info : "^infoStruct^" fields : "^string_of_info_list ia^"||--\n"
  | InfoFun (n,t,tp) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)
  | InfoType(tid,_) -> "type "^tid^"\n"

  and string_of_info_list ia = 
    match ia with
      | [] -> " --"
      | i::tl -> (string_of_info i)^string_of_info_list tl
      
let rec toStringInfo i = 
  
  match i with
    | InfoVar(n,t,_,_,f,s,ia) -> n^","^f^" : "^toStringType t^s^" fields: "^(toStringInfoList ia)^"\n"
    | _ -> failwith "Error"
    
    and toStringInfoList ia = 
        match ia with
          | [] -> " "
          | i::tl -> toStringInfo i^toStringInfoList tl
...
(* Modifie le type si c'est une InfoVar, ne fait rien sinon *)
  let modifier_type_info t i =
    match !i with
    |InfoVar (n,_,dep,base,f,opt,fields) -> i:= InfoVar (n,t,dep,base,f,opt,fields)
    |InfoType(tid,_) -> i:= InfoType(tid,t)
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_info d b i =
     match !i with
     |InfoVar (n,t,_,_,f,opt,fields) -> i:= InfoVar (n,t,d,b,f,opt,fields)
     | _ -> failwith "Appel modifier_adresse_info pas sur un InfoVar"

  let modifyLinks i ia = 
      match !i with
        |InfoVar (n,t,d,b,f,opt,_) -> i:= InfoVar (n,t,d,b,f,opt,ia)
